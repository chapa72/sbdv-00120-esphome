...
Микроконтроллер esp8266 перед впайкой на плату был заранее прошит через переходник usb-uart на базе ft232. Подключение осущствлял тонким многожильным проводом типа МГТФ. На плате ESP01 имеется два светодиода: светодиод POWER был удален, светодиод статуса (GPIO1) оставлен, настроен в конфиге как status_led.
